...
TEST(NoRecursiveParser, pre_order)
{
	//    arrange
	...
	auto parser = Server::NoRecursiveParser();
...
TEST(NoRecursiveParser, levels)
{
	...
	auto etalon = std::make_shared<std::string>("E\n+D\n+H\n++B\n++F\n+++A\n+++C\n+++G");
	//    act
	parser.levels(root,0);
	auto log = parser.Log();
	//    assert
	EXPECT_EQ(*log,*etalon);
}

TEST(NoRecursiveParser, in_order)
{
	//    arrange
	auto lA3= std::make_shared <Server::Node>("\n+++A");
	auto rC3= std::make_shared <Server::Node>("\n+++C");
...
TEST(NoRecursiveParser, post_order)
{
	//    arrange
	auto lA3= std::make_shared <Server::Node>("\n+++A");
	auto rC3= std::make_shared <Server::Node>("\n+++C");
